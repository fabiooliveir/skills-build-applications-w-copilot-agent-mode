...
export default function Workouts() {
  ...
  useEffect(() => { fetch(apiEndpoint).then((response) => response.json()).then((payload) => setWorkouts(Array.isArray(payload) ? payload : payload.results || payload.data || [])).catch((reason) => setError(reason.message)) }, [])
